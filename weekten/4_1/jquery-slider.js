...
/* Oct 4th, 2013 - Unofficial update to allow optional link and optional link target */
...
;(function($){
	...
	function panelObject($parent, id, img_url, transition, caption, link, target, on_ready) {
		var self = this;
		var img_w = 1;
		var img_h = 1;
		var tx = 0;
		...
		function initPanel() {
			...
			$display_node = link? $("<a class='smooth_slider_panel' href='" + link + "' target='" + (target || '_self') + "'></a>") : $("<div class='smooth_slider_panel'></div>");
		...
		function playlistReady() {
			...
			for (var i = 0; i < tot_images; i++) {
				...
				panels.push(new panelObject($container, i, img['url'],
						...
						img.hasOwnProperty('link')?img['link']:null,
						img.hasOwnProperty('target')?img['target']:null,
						function(panel) {
							to_load--;
